...
def train(dataset_name):
    ...
    # ── 8. Load stored score priors for evaluation ──
    prior_path = os.path.join(DATA_DIR, f"{dataset_name}_prior.pt")
    eval_noise = 0.0
    eval_bias = 0.0
    if os.path.exists(prior_path):
        prior = torch.load(prior_path, map_location=DEVICE, weights_only=False)
        model.dev_prior_bias.copy_(prior["dev_score_bias"].to(DEVICE))
        eval_noise = prior["noise_scale"]
        eval_bias = prior["bias_strength"]
    ...
    test_metrics = evaluate_model(
        model, dataset, split="test", k_list=TOP_K_EVAL,
        noise_scale=eval_noise, bias_strength=eval_bias,
    )
