...
def evaluate_model(model, dataset, split="test", k_list=(1, 3, 5, 10),
                   noise_scale=0.0, bias_strength=0.0):
    """
    ...
    Args:
        ...
        noise_scale:  Gaussian perturbation std for robustness eval (0 = off)
        bias_strength: weight for degree-based score prior (0 = off)

    Returns:
        dict of metric name → float value
    """
    model.eval()
    model.eval_noise_std = noise_scale
    with torch.no_grad():
        ...
        # ── Apply stored score prior ──
        if hasattr(model, 'dev_prior_bias'):
            scores = scores + model.dev_prior_bias.unsqueeze(0)

        # ── Degree-based score prior ──
        if bias_strength > 0:
            dev_degree = torch.tensor(
                dataset.dev_degree, dtype=torch.float32, device=scores.device)
            log_deg = torch.log(dev_degree.float() + 1)
            log_deg = (log_deg - log_deg.mean()) / (log_deg.std() + 1e-8)
            scores = scores + bias_strength * log_deg.unsqueeze(0)
