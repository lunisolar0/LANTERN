...
# ── Model ──
HIDDEN_DIM = 128          # hidden / embedding dimension for developer nodes
NUM_LAYERS_EXP = 3       # explicit bipartite propagation layers (Le)
NUM_LAYERS_IMP = 2       # implicit developer-projected layers (Li)
TOP_K_AUG = 10           # Top-K developers for semantic augmentation
TAU_MIN = 0.15           # minimum cosine similarity threshold for augmentation

# ── Training ──
EPOCHS = 500
BATCH_SIZE = 2048
LR = 2e-3
L2_REG = 1e-5            # weight decay on structural embeddings only
