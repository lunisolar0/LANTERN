...
class LANTERN(nn.Module):
    ...
    def __init__(
        ...
        # ── Evaluation robustness: Gaussian perturbation for stress testing ──
        self.eval_noise_std = 0.0

        # ── Per-developer score prior ──
        self.register_buffer("dev_prior_bias", torch.zeros(num_devs))
    ...
    def explicit_propagation(self):
        ...
        for _ in range(self.num_layers_exp):
            ...
            # ── Gaussian perturbation for robustness evaluation ──
            if self.eval_noise_std > 0 and not self.training:
                bug_h = bug_h + self.eval_noise_std * torch.randn_like(bug_h)
                dev_h = dev_h + self.eval_noise_std * torch.randn_like(dev_h)
